...
describe('toolDisplay (TUI-C30)', () => {
  ...
  /**
   * [[TUI-C106]] — the result as a LAST-RESORT source for the summary. Every case asserts on the
   * filename or on its deliberate absence; none counts lines.
   */
  describe('summariseToolCallWithResult', () => {
    const GH_RESULT = 'Full contents of acme/widgets/src/tenant/Community.ts@main:\n\nbody';

    it('names the file from the result heading when no arguments arrived', async () => {
      const { summariseToolCallWithResult } = await import('#src/core/toolDisplay.js');
      expect(summariseToolCallWithResult('gth_gh_read_file', undefined, GH_RESULT, [])).toBe(
        'gth_gh_read_file(path=acme/widgets/src/tenant/Community.ts@main)'
      );
    });

    it('names the file even when a partial args buffer left the summary at name(…)', async () => {
      const { summariseToolCallWithResult } = await import('#src/core/toolDisplay.js');
      // Two calls sharing a chunk index concatenate into a buffer that never parses. The old
      // rendering was `gth_gh_read_file(…)`, which names nothing either.
      const mixed = '{"path":"a.ts"}{"path":"b.ts"}';
      expect(summariseToolCallWithResult('gth_gh_read_file', mixed, GH_RESULT, [])).toContain(
        'src/tenant/Community.ts'
      );
    });

    it('PREFERS a real parsed args buffer over the result', async () => {
      const { summariseToolCallWithResult } = await import('#src/core/toolDisplay.js');
      // The model asked for one file and the heading names another (the binding the model is not
      // allowed to supply). The call's own arguments are what the row must say it asked for.
      expect(
        summariseToolCallWithResult('gth_gh_read_file', '{"path":"asked/for.ts"}', GH_RESULT, [])
      ).toBe('gth_gh_read_file(path=asked/for.ts)');
    });

    it('leaves a tool with no result-reader exactly as summariseToolCall left it', async () => {
      const { summariseToolCall, summariseToolCallWithResult } =
        await import('#src/core/toolDisplay.js');
      // read_file is registered but declares no `argsFromResult`, and an unknown tool has no entry
      // at all. Neither may start reading meaning out of arbitrary result text.
      expect(summariseToolCallWithResult('read_file', undefined, GH_RESULT, [])).toBe(
        summariseToolCall('read_file', undefined, [])
      );
      expect(summariseToolCallWithResult('mystery_tool', undefined, GH_RESULT, [])).toBe(
        'mystery_tool()'
      );
    });

    it('reads nothing out of a result that has no heading', async () => {
      const { summariseToolCallWithResult } = await import('#src/core/toolDisplay.js');
      expect(
        summariseToolCallWithResult('gth_gh_read_file', undefined, 'Invalid file path "x".', [])
      ).toBe('gth_gh_read_file()');
      // A heading-shaped line that never closes with its colon is not a heading.
      expect(
        summariseToolCallWithResult('gth_gh_read_file', undefined, 'Full contents of a/b.ts', [])
      ).toBe('gth_gh_read_file()');
    });

    it('redacts and neutralises a recovered label exactly like a real one', async () => {
      const { summariseToolCallWithResult } = await import('#src/core/toolDisplay.js');
      const secret = 'inline-config-secret-value';
      // The heading is tool output: it is untrusted text on the same footing as an args value, and
      // it must go through the SAME treatment rather than round it.
      const summary = summariseToolCallWithResult(
        'gth_gh_read_file',
        undefined,
        `Full contents of acme/${secret}/x.ts@main:\n\nbody`,
        [secret]
      );
